...
# then launches the v2 generate → estimate → infer pipeline.
#
# Usage:
...
echo "$(date): === Starting v2 pipeline (CUDA_VISIBLE_DEVICES=$SELECTED_GPUS) ===" | tee "$LOG"
...
$PYTHON "$SCRIPT" generate \
    ...
    --output-dir data/bulk_downloads/scripts/data/v2 \
    >> "$LOG" 2>&1
...
$PYTHON "$SCRIPT" estimate \
    ...
    --ai-corpus-dir data/bulk_downloads/scripts/data/v2 \
    --output-dir data/bulk_downloads/scripts/data/v2/ai_usage_distributions \
    >> "$LOG" 2>&1
...
$PYTHON "$SCRIPT" infer \
    --doc-types public_submission notice rule proposed_rule \
    --distribution-dir data/bulk_downloads/scripts/data/v2/ai_usage_distributions \
    --stratify-by agency quarter \
    --output data/bulk_downloads/scripts/data/v2/ai_usage_results.csv \
    >> "$LOG" 2>&1

echo "$(date): === Pipeline complete ===" | tee -a "$LOG"
echo "$(date): Results at data/bulk_downloads/scripts/data/v2/ai_usage_results.csv"
