...
# Orchestrated launcher: CoGymTravelOutcome with Qwen server + jagupard workers
# This submits ONE job that hosts the Qwen server and, once ready, submits jagupard ablation jobs back to it.
#
# Usage:
#   bash scripts/ablations/qwen/launchers/cogym_travel_outcome_orchestrated.sh
#
...
# Qwen server options (H200/H100 node) — ensure different port from EvalGen (8123)
...
echo "Submitting orchestrator for CoGymTravelOutcome (SEEDS=[$SEEDS], FULL_SUITE=$FULL_SUITE, PORT=$PORT)"
