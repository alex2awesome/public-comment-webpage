...
# Orchestrated launcher: CoGymTravelOutcome with Qwen server (jagupard TP=4) + jagupard workers
# Submits ONE job that hosts the Qwen server and, once ready, submits ablation jobs back to it.
#
# Usage:
#   bash scripts/ablations/qwen/launchers/cogym_travel_outcome_orchestrated_jag4.sh
#
...
# Qwen server options (jagupard node, TP=4) — ensure different port from EvalGen (8123)
...
echo "Submitting orchestrator for CoGymTravelOutcome (SEEDS=[$SEEDS], FULL_SUITE=$FULL_SUITE, PORT=$PORT, TP=$TP)"
