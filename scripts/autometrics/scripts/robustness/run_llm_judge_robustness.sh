...
#SBATCH --nodes=1
#SBATCH --job-name=llm_judge_robustness
#SBATCH --output=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/llm_judge_robustness_%j.out
#SBATCH --error=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/llm_judge_robustness_%j.err
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# CPU-only robustness runner for LLM Judge on unified robustness CSVs.
# Required env vars:
...
case "${LLM_MODEL}" in
  ...
  *)
    # Leave as-is; may fail choices in script if unsupported
    :;;
esac

export OPENAI_API_KEY

echo "Running LLM Judge robustness"
echo "CSV:     $CSV_PATH"
...
CACHE_TAG="robust_llmjudge_${DATASET_NAME}_${TARGET_MEASURE}_${CSV_BASENAME}_seed${SEED}"
...
scontrol update JobId=${SLURM_JOB_ID} JobName="llmjudge_${DATASET_NAME}_${TARGET_MEASURE}_${CSV_BASENAME}" >/dev/null 2>&1 || true
