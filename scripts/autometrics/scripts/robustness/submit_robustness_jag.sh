...
#SBATCH --nodes=1
#SBATCH --job-name=robust_jag
#SBATCH --output=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/robust_jag_%j.out
#SBATCH --error=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/robust_jag_%j.err
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Unified GPU robustness submitter for jagupard.
...
case "$MODE" in
  BEST)
    ...
    echo "[JAG] Running BEST Existing on GPU"
  ...
  AUTOMETRICS)
    ...
    if [ -z "${DATASET_NAME:-}" ]; then echo "❌ DATASET_NAME required"; exit 1; fi
    if [ -z "${TARGET_MEASURE:-}" ]; then echo "❌ TARGET_MEASURE required"; exit 1; fi
    echo "[JAG] Running Autometrics Static Regression on GPU (per CSV)"
    CSV_BASENAME="$(basename "${CSV_PATH}" .csv)"
