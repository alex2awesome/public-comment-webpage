...
#SBATCH --nodes=1
#SBATCH --job-name=robust_sc
#SBATCH --output=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/robust_sc_%j.out
#SBATCH --error=/nlp/scr2/nlp/personal-rm/autometrics/scripts/robustness/logs/robust_sc_%j.err
...
# Unified GPU robustness submitter for SC-loprio.
...
case "$MODE" in
  BEST)
    ...
    echo "[SC] Running BEST Existing on GPU"
  ...
  AUTOMETRICS)
    ...
    if [ -z "${DATASET_NAME:-}" ]; then echo "❌ DATASET_NAME required"; exit 1; fi
    if [ -z "${TARGET_MEASURE:-}" ]; then echo "❌ TARGET_MEASURE required"; exit 1; fi
    echo "[SC] Running Autometrics Static Regression on GPU (per CSV)"
    CSV_BASENAME="$(basename "${CSV_PATH}" .csv)"
