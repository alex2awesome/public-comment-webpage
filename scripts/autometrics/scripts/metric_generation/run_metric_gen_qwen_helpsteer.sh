...
#SBATCH --nodes=1
#SBATCH --job-name=metric_gen_qwen_helpsteer
#SBATCH --output=scripts/metric_generation/logs/metric_gen_qwen_helpsteer.out
#SBATCH --error=scripts/metric_generation/logs/metric_gen_qwen_helpsteer.err
#SBATCH --constraint=141G
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Script for HelpSteer dataset with Qwen3-32B
...
port=7410  # Use standard port for Qwen
model_nickname="qwen3_32b"

echo "Starting Qwen3-32B server for HelpSteer metric generation..."
...
export DSPY_CACHEDIR="/nlp/scr3/nlp/20questions/dspy_cache/autometrics_metric_gen_qwen_helpsteer"
export AUTOMETRICS_MODEL_DIR="/sphinx/u/salt-checkpoints/autometrics/models"

# Set API base URL
API_BASE=http://localhost:${port}/v1

echo "Starting Metric Generation Benchmark with Qwen3-32B for HelpSteer dataset..."
...
# Run the benchmark for HelpSteer dataset only
...
echo "HelpSteer dataset benchmark completed with Qwen3-32B!"
...
echo "  - HelpSteer: helpfulness, correctness, coherence, complexity, verbosity"
echo ""
