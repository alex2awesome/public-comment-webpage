...
#SBATCH --nodes=1
#SBATCH --job-name=metric_gen_qwen_helpsteer_helpfulness
#SBATCH --output=scripts/metric_generation/logs/metric_gen_qwen_helpsteer_helpfulness.out
#SBATCH --error=scripts/metric_generation/logs/metric_gen_qwen_helpsteer_helpfulness.err
...
echo "Starting Qwen3-32B server for HelpSteer (helpfulness)..."
...
export DSPY_CACHEDIR="/nlp/scr3/nlp/20questions/dspy_cache/autometrics_metric_gen_qwen_helpsteer_helpfulness"
export AUTOMETRICS_MODEL_DIR="/sphinx/u/salt-checkpoints/autometrics/models"

echo "Starting Metric Generation Benchmark with Qwen3-32B for HelpSteer (helpfulness)..."
...
echo "HelpSteer (helpfulness) benchmark completed with Qwen3-32B!"
