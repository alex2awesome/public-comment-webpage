...
#SBATCH --nodes=1
#SBATCH --job-name=qwen_scaling_genonly_cpu
#SBATCH --output=logs/qwen_scaling_genonly_cpu_%j.out
#SBATCH --error=logs/qwen_scaling_genonly_cpu_%j.err
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Data scaling run (Generated-only mode) on CPU, pointing to an existing Qwen server (set QWEN_API_BASE).
...
echo "Running Qwen data scaling (GENERATED-ONLY, CPU)"
...
ensure_caches() {
  ...
  if [ ! -d "${AUTOMETRICS_CACHE_DIR}" ]; then
    if [ -d "${src_auto}" ]; then
      echo "[Scaling GENONLY] Cloning Autometrics cache from ${src_auto} → ${AUTOMETRICS_CACHE_DIR}"
      mkdir -p "$(dirname "${AUTOMETRICS_CACHE_DIR}")"
      cp -r "${src_auto}" "${AUTOMETRICS_CACHE_DIR}"
    else
      echo "[Scaling GENONLY] No source Autometrics cache found at ${src_auto}; starting fresh."
    fi
  fi

  if [ ! -d "${DSPY_CACHEDIR}" ]; then
    if [ -d "${src_dspy}" ]; then
      echo "[Scaling GENONLY] Cloning DSPy cache from ${src_dspy} → ${DSPY_CACHEDIR}"
      mkdir -p "$(dirname "${DSPY_CACHEDIR}")"
      cp -r "${src_dspy}" "${DSPY_CACHEDIR}"
    else
      echo "[Scaling GENONLY] No source DSPy cache found at ${src_dspy}; starting fresh."
    fi
  fi
}
...
echo "[Scaling GENONLY] Using TMPDIR=${TMPDIR}"
