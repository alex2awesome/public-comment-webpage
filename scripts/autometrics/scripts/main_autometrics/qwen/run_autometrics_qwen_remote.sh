...
#SBATCH --nodes=1
#SBATCH --job-name=qwen_main_remote
#SBATCH --output=logs/qwen_main_remote_%j.out
#SBATCH --error=logs/qwen_main_remote_%j.err
#SBATCH -x jagupard[19-20,26-31]
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Main run on jagupard GPUs pointing to an existing Qwen server (set QWEN_API_BASE).
...
echo "[Main Remote] Using TMPDIR=${TMPDIR}"
...
echo "[Main Remote] TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR}"
echo "[Main Remote] TRITON_CACHE_DIR=${TRITON_CACHE_DIR}"
