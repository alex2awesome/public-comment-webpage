...
#SBATCH --nodes=1
#SBATCH --job-name=qwen_main_orchestrator_jag4
#SBATCH --output=logs/qwen_main_orchestrator_jag4_%j.out
#SBATCH --error=logs/qwen_main_orchestrator_jag4_%j.err
#SBATCH -x jagupard[19-20,26-31]
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Orchestrator for main runs on jagupard: 4-way tensor parallel.
...
echo "[Main Orchestrator JAG4] Using TMPDIR=${TMPDIR}"
...
echo "[Main Orchestrator JAG4] TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR}"
echo "[Main Orchestrator JAG4] TRITON_CACHE_DIR=${TRITON_CACHE_DIR}"
...
echo "[Main Orchestrator JAG4] Starting Qwen server on ${HOST}:${PORT}"
...
echo "[Main Orchestrator JAG4] Waiting for server readiness at ${API_BASE} …"
...
until curl -s "${API_BASE}/get_model_info" > /dev/null; do
  if ! kill -0 ${SERVER_PID} 2>/dev/null; then
    echo "[Main Orchestrator JAG4] ❌ Server process exited unexpectedly."
    tail -n 100 "${SERVER_LOG}" || true
    kill ${TAIL_PID} 2>/dev/null || true
    exit 1
  fi
  echo "[Main Orchestrator JAG4] Still waiting …"
  sleep 10
  if [ $(( $(date +%s) - START_TIME )) -gt $((TIMEOUT * 60)) ]; then
    echo "[Main Orchestrator JAG4] ❌ Timeout waiting for server."
    tail -n 100 "${SERVER_LOG}" || true
    kill ${TAIL_PID} 2>/dev/null || true
...
echo "[Main Orchestrator JAG4] ✅ Server ready at ${API_BASE}"
kill ${TAIL_PID} 2>/dev/null || true
...
echo "[Main Orchestrator JAG4] Submitting main jobs (one per seed)"
...
submit_one() {
  ...
  if [ -f "${log_file}" ] && [ -f "${score_file}" ]; then
    echo "[Main Orchestrator JAG4] ✅ Already done: ${DATASET_NAME}/${TARGET_MEASURE} seed=${seed}"
    return 0
  fi
  ...
  echo "[Main Orchestrator JAG4] Submitted job_name=${job_name} seed=${seed} => job ${jid}"
}

for s in ${SEEDS}; do submit_one "$s"; done

echo "[Main Orchestrator JAG4] Server will remain running. Cancel this job to shut it down (scancel ${SLURM_JOB_ID})."
echo "[Main Orchestrator JAG4] QWEN_API_BASE=${API_BASE}"
