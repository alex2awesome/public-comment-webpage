...
#SBATCH --nodes=1
#SBATCH --job-name=qwen_main_orchestrator
#SBATCH --output=logs/qwen_main_orchestrator_%j.out
#SBATCH --error=logs/qwen_main_orchestrator_%j.err
#SBATCH --constraint=[141G|80G]
#SBATCH --requeue
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=mryan0@stanford.edu

# Orchestrator for main runs: starts a persistent Qwen server and submits
# main jobs that connect to it via QWEN_API_BASE.
...
echo "[Main Orchestrator] Using TMPDIR=${TMPDIR}"
...
echo "[Main Orchestrator] TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR}"
echo "[Main Orchestrator] TRITON_CACHE_DIR=${TRITON_CACHE_DIR}"
...
echo "[Main Orchestrator] Waiting for server readiness at ${API_BASE} …"
...
until curl -s "${API_BASE}/get_model_info" > /dev/null; do
  if ! kill -0 ${SERVER_PID} 2>/dev/null; then
    echo "[Main Orchestrator] ❌ Server process exited unexpectedly."
    tail -n 100 "${SERVER_LOG}" || true
    kill ${TAIL_PID} 2>/dev/null || true
    exit 1
  fi
  echo "[Main Orchestrator] Still waiting …"
  sleep 10
  if [ $(( $(date +%s) - START_TIME )) -gt $((TIMEOUT * 60)) ]; then
    echo "[Main Orchestrator] ❌ Timeout waiting for server."
    tail -n 100 "${SERVER_LOG}" || true
    kill ${TAIL_PID} 2>/dev/null || true
...
echo "[Main Orchestrator] ✅ Server ready at ${API_BASE}"
kill ${TAIL_PID} 2>/dev/null || true
...
echo "[Main Orchestrator] Submitting main jobs (one per seed)"
...
submit_one() {
  ...
  if [ -f "${log_file}" ] && [ -f "${score_file}" ]; then
    echo "[Main Orchestrator] ✅ Already done: ${DATASET_NAME}/${TARGET_MEASURE} seed=${seed}"
    return 0
  fi
  ...
  jid=$(sbatch \
    ...
    --export=${envs} \
    scripts/main_autometrics/qwen/run_autometrics_qwen_remote_sc.sh | awk '{print $4}')
  echo "[Main Orchestrator] Submitted job_name=${job_name} seed=${seed} => job ${jid}"
}

for s in ${SEEDS}; do submit_one "$s"; done

echo "[Main Orchestrator] Server will remain running. Cancel this job to shut it down (scancel ${SLURM_JOB_ID})."
echo "[Main Orchestrator] QWEN_API_BASE=${API_BASE}"
