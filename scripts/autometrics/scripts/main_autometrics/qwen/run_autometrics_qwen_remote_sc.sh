...
#SBATCH --nodes=1
#SBATCH --job-name=qwen_main_remote_sc
#SBATCH --output=logs/qwen_main_remote_sc_%j.out
#SBATCH --error=logs/qwen_main_remote_sc_%j.err
...
# Main run on SC GPUs pointing to an existing Qwen server (set QWEN_API_BASE).
...
echo "[Main Remote SC] Using TMPDIR=${TMPDIR}"
...
echo "[Main Remote SC] TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR}"
echo "[Main Remote SC] TRITON_CACHE_DIR=${TRITON_CACHE_DIR}"
