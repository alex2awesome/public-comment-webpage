...
#SBATCH --job-name=llm_judge_gpt4o_helpsteer2
#SBATCH --output=scripts/llm_judge/logs/llm_judge_gpt4o_helpsteer2.out
#SBATCH --error=scripts/llm_judge/logs/llm_judge_gpt4o_helpsteer2.err
...
# GPT-4o-mini LLM Judge Correlation Analysis - HelpSteer Dataset
# This script runs LLM judge correlation analysis specifically for HelpSteer datasets
...
echo "Starting GPT-4o-mini LLM Judge correlation analysis for HelpSteer datasets..."

# Set DSPy cache directory (separate from others to avoid conflicts)
export DSPY_CACHEDIR="./.cache/dspy_llm_judge_gpt4o_helpsteer"

# Ensure output directories exist
mkdir -p results/main_runs/baselines/llm_judge_sub_results

# Run all three correlation types for HelpSteer datasets
echo "Running correlation analysis for HelpSteer datasets..."
...
echo "GPT-4o-mini LLM Judge correlation analysis for HelpSteer datasets completed!"
