...
#SBATCH --job-name=llm_judge_gpt4o_simplification
#SBATCH --output=scripts/llm_judge/logs/llm_judge_gpt4o_simplification.out
#SBATCH --error=scripts/llm_judge/logs/llm_judge_gpt4o_simplification.err
#SBATCH --time=6:00:00
...
# GPT-4o-mini LLM Judge Correlation Analysis - Simplification Datasets
# This script runs LLM judge correlation analysis specifically for simplification datasets
...
echo "Starting GPT-4o-mini LLM Judge correlation analysis for simplification datasets..."

# Set DSPy cache directory (separate from others to avoid conflicts)
export DSPY_CACHEDIR="./.cache/dspy_llm_judge_gpt4o_simplification"
...
echo "Running correlation analysis for simplification datasets..."
...
echo "GPT-4o-mini LLM Judge correlation analysis for simplification datasets completed!"
...
try:
    ...
    if not df.empty:
        ...
        # Filter for simplification datasets only
        df = df[df['dataset'].isin(['SimpDA', 'SimpEval'])]
        if not df.empty:
