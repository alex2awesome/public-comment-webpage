...
#SBATCH --job-name=dna_eval_gpt5_iclr
#SBATCH --output=scripts/dna_eval/logs/dna_eval_gpt5_iclr.out
#SBATCH --error=scripts/dna_eval/logs/dna_eval_gpt5_iclr.err
#SBATCH --time=6:00:00
...
# GPT-5-mini DNAEval Correlation Analysis - ICLR Dataset
...
echo "Starting GPT-5-mini DNAEval correlation analysis for ICLR dataset..."

# Set DSPy cache directory (separate from others to avoid conflicts)
export DSPY_CACHEDIR="./.cache/dspy_dna_eval_gpt5_iclr"

# Ensure output directories exist
mkdir -p results/main_runs/baselines/dna_eval_sub_results
mkdir -p scripts/dna_eval/logs

# Run all three correlation types for ICLR
echo "Running correlation analysis for ICLR..."
...
echo "GPT-5-mini DNAEval correlation analysis for ICLR dataset completed!"
...
for corr in kendall pearson spearman; do
    ...
    if [ -f "results/main_runs/baselines/dna_eval_gpt5_mini_${corr}.csv" ]; then
        python3 -c "
import pandas as pd
try:
    df = pd.read_csv('results/main_runs/baselines/dna_eval_gpt5_mini_${corr}.csv')
    if not df.empty:
